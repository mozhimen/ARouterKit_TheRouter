...
fun init(ctx: Context) {
    UtilKLogWrapper.i("AppFlowTask.TEST_LATE_1", "packageName:${ctx.packageName},这是延迟初始化的代码1")
}

@FlowTask(taskName = AppFlowTask.TEST_LATE_2, dependsOn = TheRouterFlowTask.THEROUTER_INITIALIZATION)
fun init2(ctx: Context) {
    UtilKLogWrapper.i("AppFlowTask.TEST_LATE_2", "packageName:${ctx.packageName},这是延迟初始化的代码2")
}